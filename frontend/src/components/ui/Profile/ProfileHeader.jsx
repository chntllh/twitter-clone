...
const ProfileHeader = ({ user, isOwner }) => {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute top-32 left-4 w-32 h-32 rounded-full border-4 border-black overflow-hidden">
        <img src={user.avatarUrl} alt="Avatar" className="w-32"/>
      </div>

      {isOwner && (
        <button className="absolute right-4 top-52 px-4 py-2 hover:bg-neutral-800 border border-neutral-600 font-bold rounded-full">
          Edit profile
        </button>
      )}
