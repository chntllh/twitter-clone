...
export const validateDisplayName = async (displayName: string) => {
  ...
  if (
    ...
    throw errorHandler(
      400,
      `Name must be between ${DISPLAY_NAME_MIN_LENGTH} and ${DISPLAY_NAME_MAX_LENGTH} characters.`,
      {
        code: "NAME_LENGTH_ERROR",
        description: `Name must be between ${DISPLAY_NAME_MIN_LENGTH} and ${DISPLAY_NAME_MAX_LENGTH} characters.`,
        field: "displayName",
      }
    );
  }
  ...
  if (bio.length > BIO_MAX_LENGTH) {
    throw errorHandler(
      ...
      `Bio must not exceed ${BIO_MAX_LENGTH} characters.`,
      {
        ...
        description: `Bio must not exceed ${BIO_MAX_LENGTH} characters.`,
        field: "bio",
      }
    );
  }
...
export const validatePassword = async (
  ...
  if (newPassword.length < PASSWORD_MIN_LENGTH) {
    throw errorHandler(
      400,
      `Password must be over ${PASSWORD_MIN_LENGTH} characters`,
      {
        code: "PASSWORD_TOO_SHORT",
        description: `Password must be over ${PASSWORD_MIN_LENGTH} characters`,
        field: "newPassword",
      }
    );
  }
